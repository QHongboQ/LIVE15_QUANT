...
FEATURE_SETS: dict[str, tuple[str, ...]] = {
    "A0": (),
    "A1": (
        "return_15s", "return_30s", "return_60s", "return_120s", "return_300s",
        "return_momentum", "return_acceleration", "realized_volatility_60s",
        "realized_volatility_120s", "realized_volatility_300s", "price_range_60s",
        "volatility_change", "volatility_regime_ratio",
    ),
    "A2": (
        "return_15s", "return_30s", "return_60s", "return_120s", "return_300s",
        "return_momentum", "return_acceleration", "realized_volatility_60s",
        "realized_volatility_120s", "realized_volatility_300s", "price_range_60s",
        "volatility_change", "volatility_regime_ratio", "signed_distance_to_target",
    ...
    "A3": (
        ...
        "realized_volatility_120s", "realized_volatility_300s", "price_range_60s",
        "volatility_change", "volatility_regime_ratio", "signed_distance_to_target",
        "normalized_distance_to_target", "time_remaining_seconds",
    ),
    "A4": (
        "return_15s", "return_30s", "return_60s", "return_120s", "return_300s",
        "return_momentum", "return_acceleration", "realized_volatility_60s",
        "realized_volatility_120s", "realized_volatility_300s", "price_range_60s",
        "volatility_change", "volatility_regime_ratio", "signed_distance_to_target",
        "normalized_distance_to_target", "time_remaining_seconds", "yes_bid", "yes_ask",
        "no_bid", "no_ask", "yes_spread", "yes_midpoint", "market_probability_midpoint",
...
def _load_rows(root: Path, event_splits: dict[str, str]) -> tuple[dict[tuple[str, datetime], dict[str, Any]], Counter[str]]:
...
def _load_targets(root: Path, event_splits: dict[str, str]) -> tuple[dict[tuple[str, datetime, int | str], dict[str, Any]], Counter[str], Counter[str]]:
...
def load_examples(root: Path) -> tuple[tuple[Example, ...], dict[str, Any]]:
    ...
    for (event, decision, horizon), target in targets.items():
        ...
        examples.append(
            Example(
                event, row["asset"], row["split"], decision, row["window_start"], row["window_end"],
                horizon, row["features"], target["return"], int(target["return"] > 0),
            )
        )
    ...
    return tuple(sorted(examples, key=lambda item: (item.decision, item.event, str(item.horizon))),), metadata
...
def _folds(examples: Sequence[Example], count: int = 3) -> tuple[Fold, ...]:
    ...
    for item in examples:
        (train_windows if item.split == "train" else validation_windows)[item.window_start].append(item)
    ...
    for index in range(count):
        validation_slice = tuple(ordered_validation[index * width : (index + 1) * width] if index < count - 1 else ordered_validation[index * width :])
        ...
        max_target = max(row.decision + timedelta(seconds=int(row.horizon) if isinstance(row.horizon, int) else 0) for row in train)
        if min(row.decision for row in validation) < max_target + timedelta(seconds=PURGE_EMBARGO_SECONDS):
            continue
...
class _Norm:
    def __init__(self, names: tuple[str, ...], med: np.ndarray, mean: np.ndarray, scale: np.ndarray):
        self.names, self.med, self.mean, self.scale = names, med, mean, scale

    @classmethod
    def fit(cls, rows: Sequence[Example], names: tuple[str, ...]) -> _Norm:
        matrix = np.asarray([[row.features.get(name, np.nan) for name in names] for row in rows], dtype=float)
    ...
    def transform(self, rows: Sequence[Example]) -> np.ndarray:
        matrix = np.asarray([[row.features.get(name, np.nan) for name in self.names] for row in rows], dtype=float)
...
def _fit_predict(model: str, train: Sequence[Example], valid: Sequence[Example], names: tuple[str, ...]) -> tuple[np.ndarray, np.ndarray]:
    ...
    if model == "naive":
        ...
        if model == "linear":
            ...
            coef = np.linalg.solve(design.T @ design + np.eye(design.shape[1]) * 1e-6, design.T @ actual)
            prediction = np.column_stack((np.ones(len(xv)), xv)) @ coef
        elif model == "logistic":
            labels = (actual > 0).astype(float)
            weights = np.zeros(x.shape[1], dtype=float)
            intercept = float(np.log(np.clip(np.mean(labels), 1e-6, 1 - 1e-6) / np.clip(1 - np.mean(labels), 1e-6, 1)))
            for _ in range(350):
            ...
            prediction = (1.0 / (1.0 + np.exp(-np.clip(xv @ weights + intercept, -30, 30))) - 0.5) * 1e-4
        elif model == "xgboost":
            from live15_quant.model_zoo import xgb

            booster = xgb.train(
                {"objective": "reg:squarederror", "eval_metric": "rmse", "seed": SEED,
                 "nthread": 1, "tree_method": "hist", "max_depth": 3, "eta": 0.05,
                 "min_child_weight": 8, "lambda": 5.0},
                xgb.DMatrix(x, label=actual), num_boost_round=40,
            )
...
def _metrics(rows: Sequence[Example], prediction: np.ndarray, probability: np.ndarray) -> dict[str, Any]:
    actual = np.asarray([row.target_return for row in rows], dtype=float)
    labels = (actual > 0).astype(int)
    predicted = (probability >= 0.5).astype(int)
    positive, negative = labels == 1, labels == 0
    recalls = [float(np.mean(predicted[mask] == labels[mask])) for mask in (positive, negative) if np.any(mask)]
    ...
    for left, right in zip(bins[:-1], bins[1:], strict=True):
        ...
        if np.any(mask):
            ece += float(np.mean(mask)) * abs(float(np.mean(probability[mask])) - float(np.mean(labels[mask])))
    rank_x, rank_y = np.argsort(np.argsort(prediction)), np.argsort(np.argsort(actual))
    return {
        "examples": len(rows), "mae": float(np.mean(np.abs(prediction - actual))),
        ...
        "logloss": float(-np.mean(labels * np.log(np.clip(probability, 1e-9, 1 - 1e-9)) + (1 - labels) * np.log(np.clip(1 - probability, 1e-9, 1 - 1e-9)))),
        "brier": float(np.mean((probability - labels) ** 2)), "ece": ece,
        "spearman": float(np.corrcoef(rank_x, rank_y)[0, 1]) if len(rows) > 1 else None,
    }


def _aggregate(rows: Sequence[Example], model: str, names: tuple[str, ...], folds: Sequence[Fold]) -> dict[str, Any]:
    ...
    per_asset = {asset: _metrics((subset := tuple(row for row in validation_rows if row.asset == asset)), pred_all[[i for i, row in enumerate(validation_rows) if row.asset == asset]], prob_all[[i for i, row in enumerate(validation_rows) if row.asset == asset]]) for asset in sorted({row.asset for row in validation_rows}) if sum(row.asset == asset for row in validation_rows) >= 2}
    per_day = {day: _metrics((subset := tuple(row for row in validation_rows if row.decision.astimezone(UTC).date().isoformat() == day)), pred_all[[i for i, row in enumerate(validation_rows) if row.decision.astimezone(UTC).date().isoformat() == day]], prob_all[[i for i, row in enumerate(validation_rows) if row.decision.astimezone(UTC).date().isoformat() == day]]) for day in sorted({row.decision.astimezone(UTC).date().isoformat() for row in validation_rows}) if sum(row.decision.astimezone(UTC).date().isoformat() == day for row in validation_rows) >= 2}
    payload = {"model": model, "folds": fold_metrics, "pooled": _metrics(validation_rows, pred_all, prob_all), "per_asset": per_asset, "per_day": per_day}
    payload["artifact_identity"] = hashlib.sha256(json.dumps({"model": model, "features": names, "seed": SEED, "dataset_id": DATASET_ID, "build_hash": BUILD_HASH, "folds": fold_metrics}, sort_keys=True).encode()).hexdigest()
    return payload
...
def evaluate(root: Path) -> dict[str, Any]:
    ...
    for horizon in HORIZONS:
        ...
        for feature_set, requested in FEATURE_SETS.items():
            names = tuple(name for name in requested if any(name in row.features for row in horizon_rows))
            models = ("naive",) if feature_set == "A0" else ("linear", "logistic", "xgboost")
            for model in models:
                result = _aggregate(horizon_rows, model, names, folds)
                result.update({"feature_set": feature_set, "horizon": horizon, "feature_names": names})
                candidates.append(result)
    prior = json.loads((Path(__file__).parents[1] / "docs" / "model_vnext_mvn002_report.json").read_text(encoding="utf-8"))
    prior_30 = next((item for item in prior["candidates"] if item["horizon"] == 30 and item["model"] == "xgboost" and item["feature_set"] == "A2"), None)
    current_30 = next((item for item in candidates if item["horizon"] == 30 and item["model"] == "xgboost" and item["feature_set"] == "A2"), None)
    ...
    report = {
        "status": status, "experiment": "MVN-002R", "development_only": True,
        "dataset_id": DATASET_ID, "build_hash": BUILD_HASH,
        "cutoff_timestamp": metadata["manifest"]["cutoff"]["registered_at"],
        "independent_utc_days": metadata["independent_utc_days"], "independent_events": metadata["independent_events"],
        "row_counts": metadata["row_counts"], "horizons": list(HORIZONS),
        "target_counts": metadata["target_counts"], "unavailable_counts": metadata["unavailable_counts"],
        "feature_sets": {key: list(value) for key, value in FEATURE_SETS.items()}, "models": list(MODELS),
        "candidates": candidates, "prior_dataset_v1": {"dataset_id": prior["dataset_id"], "xgb_a2_30s_directional_accuracy": prior_accuracy},
        "dataset_v2_xgb_a2_30s_directional_accuracy": current_accuracy,
        "v1_vs_v2_30s_result": "UNSTABLE_OR_NO_ROBUST_EDGE" if current_accuracy is None else ("STRENGTHENED" if current_accuracy > prior_accuracy + 0.02 else "WEAKENED_OR_REPLICATED"),
        "holdout_rows_consumed": False, "holdout_labels_loaded": False,
        "leakage_checker": "PASS", "purge_embargo_seconds": PURGE_EMBARGO_SECONDS,
        "sequence_gate": "INSUFFICIENT_SEQUENCE_EVIDENCE", "microstructure_gate": "INSUFFICIENT_MICROSTRUCTURE_EVIDENCE",
        "robust_edge_criteria": ["positive consistency across chronological folds, days, and assets", "advantage over naive and linear without isolated-fold selection", "probability quality not worse than baseline"],
...
def main() -> None:
    ...
    print(json.dumps({key: report[key] for key in ("status", "dataset_id", "target_counts", "unavailable_counts", "holdout_rows_consumed", "leakage_checker", "dataset_v2_xgb_a2_30s_directional_accuracy")}, sort_keys=True))
