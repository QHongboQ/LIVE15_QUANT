...
Approved direction: incremental infrastructure replacement, not a LIVE15 domain rewrite. Preserve the Kalshi SDK gateway, Recorder/RecorderStore and WAL evidence, settlement truth, strict as-of and gap/resync policy, FastAPI routes and typed projections, Hard Risk, Execution, Production gates, and the XGBoost baseline. Keep Nomad + Windows SCM as the highest-priority validated runtime candidate. Keep React Admin + Material UI, Vector or OpenTelemetry (one choice), Prometheus/Alertmanager/Grafana, Task + uv/dev-container, Polars/Arrow/DuckDB, NATS, Caddy, ONNX Runtime, MLflow, Optuna, and DVC as separately gated candidates; require measurement, license/security review, adapter boundaries, and rollback evidence. Do not create a second Project Brain, move Recorder truth into a broker, access holdout, alter Hard Risk/Execution, train/promote models, or connect Nomad to Production. Use one task ID/worktree, frozen contracts, shadow evidence, and distinct MERGED/DEPLOYED/VERIFIED states. See `docs/roadmap/UPSTREAM_REPLACEMENT_MATRIX_001.md`.