...
| NOMAD-FIRST-WORKLOAD-SHADOW-001 | LOCAL_ACCEPTANCE_COMPLETE / REVIEW_PENDING | External Windows administrator initialization set trusted owner/top DACL; non-elevated verification passed for all ten POC objects, immutable hashes, no reparse/Production reference, and sealed preflight. Nomad natively restored allocation `2eb3bf4a` under deployment `a27379da`: running, native check success, 127.0.0.1:18081 health 200, restarts 0. Exact evidence: `docs/deployment/NOMAD_FIRST_WORKLOAD_SHADOW_001.md`. Generic POC long checks were not replayed. | Maker/Checker then one PR; no cutover or Recorder migration |
