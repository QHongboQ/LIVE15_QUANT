...
- Resolve `origin/main` at task start; this index is reconciled after PR #95.
  `MERGED != DEPLOYED`.
...
| NOMAD-MIGRATION-STATUS-20260830 | SHADOW MERGED / CUTOVER-RESUME BLOCKED | PR #85, #86 and #91 are merged; shadow acceptance passed, but the real cutover awaits its secure immutable artifact. Detail: `docs/project-brain/NOMAD_MIGRATION_STATUS_20260830.md`. | Actual ControlCenter deployment/restart/cutover still requires explicit human approval |
| NOMAD-CONTROL-CENTER-CUTOVER-RESUME-001 | BLOCKED / immutable ControlCenter artifact prerequisite | Protected CPython 3.13.15 runtime and a purged Nomad LocalService import preflight PASS; the full lock closure now pins `tcmlib==1.5.0`. No non-user-writable LocalService-readable application-release root/artifact exists, so no owner switch was attempted. Detail: `docs/deployment/NOMAD_CONTROL_CENTER_CUTOVER_RESUME_001.md`. | After this lock correction is reviewed/merged, operator must stage the verified clean-SHA `DEP-PKG-001` artifact in a dedicated secure release root. |
