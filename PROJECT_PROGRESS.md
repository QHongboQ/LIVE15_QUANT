...
| NOMAD-MIGRATION-STATUS-20260830 | RUNTIME_ACCEPTED / GOVERNANCE_PENDING | Nomad owns the ControlCenter allocation from immutable `b1e1894`; WinSW is stopped and retained as rollback. Maker, Checker, and CI remain before final VERIFIED. Detail: `docs/project-brain/NOMAD_MIGRATION_STATUS_20260830.md`. | No retirement or Recorder change without separate approval |
| NOMAD-CONTROL-CENTER-CUTOVER-FINAL-001 | RUNTIME_ACCEPTED / GOVERNANCE_PENDING | Immutable release, native `/api/health`, listener ownership, no duplicate port, and WinSW rollback boundary passed. Detail: `docs/deployment/NOMAD_CONTROL_CENTER_CUTOVER_FINAL_001.md`. | Final VERIFIED needs Maker, Checker, and CI; WinSW retirement is separately authorized. |
