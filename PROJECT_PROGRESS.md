...
| NOMAD-FIRST-WORKLOAD-SHADOW-001 | BLOCKED / TRUSTED_ACL_OWNER_REQUIRED | A no-credential ControlCenter shadow has a passing bounded Nomad liveness observation, but the staged root and children are owned by the interactive user. The stager now fails closed until BUILTIN\Administrators owns the full staging tree. Exact evidence: `docs/deployment/NOMAD_FIRST_WORKLOAD_SHADOW_001.md`. Generic POC long checks were not replayed. | Human must pre-provision approved POC ACL owner without widening to Production/UAC; no cutover or Recorder migration |
