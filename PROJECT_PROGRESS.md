...
| DEP-PKG-001 | Auditable SHA-pinned Production release pipeline / deployment | IN_PROGRESS | `docs/deployment/PRODUCTION_RELEASE_PIPELINE_001.md`; offline package/activation/rollback tests | Establish the DEP-001 prerequisite only; no Production release capture, activation, or restart in this task. | Deployment/restart |
| DEP-001 | Current-main Production deployment and bounded proof / deployment | BLOCKED | Depends on DEP-PKG-001 merged validation; current installed package still cannot be tied to protected `origin/main` | After DEP-PKG-001 merge, rerun preflight; with explicit human approval, deploy and prove the then-current reviewed protected main. | Deploy/restart |
