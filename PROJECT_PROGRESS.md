...
| NOMAD-MIGRATION-STATUS-20260830 | SHADOW MERGED / CUTOVER_PREP PLANNED | PR #85, #86 and #91 are merged; the first read-only ControlCenter shadow passed its bounded acceptance. Full current state and next task: `docs/project-brain/NOMAD_MIGRATION_STATUS_20260830.md`. | Actual ControlCenter deployment/restart/cutover still requires explicit human approval |
| NOMAD-CONTROL-CENTER-CUTOVER-001 | BLOCKED / LocalService runtime prerequisite | Task-time official-source and host audit: clean SHA code packaging exists, but no non-user-writable, LocalService-readable Python runtime contains the locked ControlCenter dependencies. No service change. Detail: `docs/deployment/NOMAD_CONTROL_CENTER_CUTOVER_001.md`. | Approved Windows installation process must provision that runtime and hashes before a Nomad allocation can start. |
