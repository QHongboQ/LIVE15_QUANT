...
| NOMAD-POC-VALIDATE-001 | PR_OPEN / lifecycle cycle 5 reconciled | PR #71 (`agent/nomad-lifecycle-observer-001`); Branch `agent/nomad-lifecycle-observer-001`; upstream: HashiCorp Nomad job/allocation/check APIs (`https://developer.hashicorp.com/nomad/docs/concepts/job`, `https://developer.hashicorp.com/nomad/api-docs/allocations`, `https://developer.hashicorp.com/nomad/docs/job-specification/check`). The bounded POC observer delegates restart/health to Nomad. Cycles 1–4 and completed crash/health matrices were inherited; cycle 5 saw one transient empty allocation snapshot after native `job restart`, then passed on attempt 2 with the allocation healthy. Evidence: `D:\LIVE15_NOMAD_POC\generic-poc\logs\lifecycle-cycle5-reconciliation.log`. Dependency: separate `NOMAD-POC-AGENT-RESTART-001` five-cycle rediscovery validation. Blocker: Independent Checker remains blocked on legacy Codex role-TOML schema repair. | No Production cutover; POC only |
| NOMAD-POC-AGENT-RESTART-001 | BLOCKED / forced-restart false pass prevented | The first forced Windows agent restart left `raw_exec` unable to reattach and an orphaned POC fixture process (PID 18348) registered in HTTP.sys on port 18080. The five log entries at `D:\LIVE15_NOMAD_POC\generic-poc\logs\agent-restart-rediscovery-matrix.log` are preserved as diagnostic evidence, not proof: the allocation is pending after 100 restarts and the durable checkpoint is superseded by current authoritative allocation state. The observer now requires running task state plus two spaced, consecutive Nomad-native healthy observations. Upstream corroboration: HashiCorp Nomad #16922 and #23668; a supported Windows-service boundary must be prepared and independently validated before another agent-restart matrix. | POC only; do not rerun forced-agent evidence or alter Production services |
