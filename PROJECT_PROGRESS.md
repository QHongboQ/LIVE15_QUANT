...
The approved direction is an incremental infrastructure replacement, not a rewrite of LIVE15's domain core. Preserve the Kalshi SDK gateway, Recorder/RecorderStore and SQLite/WAL evidence, settlement truth, strict as-of and gap/resync policy, FastAPI routes and typed projection contracts, Hard Risk, Execution, Production gates, and the current XGBoost baseline. Replace only unstable generic infrastructure around those contracts: WinSW/custom runtime supervision with Nomad + Windows SCM; the custom web shell with React Admin + Material UI while retaining the FastAPI API; custom telemetry with Vector plus Prometheus/Alertmanager/Grafana; and inconsistent local commands/environments with a reviewed Taskfile plus pinned uv/dev-container workflow. Evaluate Polars/Arrow/DuckDB, NATS JetStream, Caddy, ONNX Runtime, MLflow, Optuna, and DVC only as separately gated candidates after measurement and license/security review. Do not introduce a second Project Brain, move Recorder truth into a broker, access holdout data, alter Hard Risk/Execution, train or promote models, or connect Nomad to Production. Migration must use one task ID per branch/worktree, frozen API/data contracts, adapter boundaries, parallel/shadow evidence, rollback, and distinct states for MERGED, DEPLOYED, and VERIFIED. The full decision matrix is `docs/roadmap/UPSTREAM_REPLACEMENT_MATRIX_001.md`; this remains a docs-only decision record pending Maker/local validation and Independent Checker re-review.
