...
| NOMAD-FIRST-WORKLOAD-SHADOW-001 | BLOCKED_HUMAN_GATE / OWNER_REPAIR_UNAVAILABLE | A no-credential ControlCenter shadow has a passing bounded Nomad liveness observation, but the staged root and children are owned by the interactive user. The first authorized fixed `icacls /setowner BUILTIN\Administrators /T /C` command was denied for all 10 POC entries; a later authorized wrapper failed to parse before native ACL execution. Both post-checks confirmed no owner, DACL, or hash change. Exact evidence: `docs/deployment/NOMAD_FIRST_WORKLOAD_SHADOW_001.md`. Generic POC long checks were not replayed. | External human action required; no further UAC, no DACL change, no allocation restore, no cutover or Recorder migration |
