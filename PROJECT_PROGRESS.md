...
| NOMAD-FIRST-WORKLOAD-SHADOW-001 | BLOCKED_HUMAN_GATE / OWNER_REPAIR_ACCESS_DENIED | A no-credential ControlCenter shadow has a passing bounded Nomad liveness observation, but the staged root and children are owned by the interactive user. After a read-only boundary preflight and Nomad-native job stop, the one authorized fixed `icacls /setowner BUILTIN\Administrators /T /C` command was denied for all 10 POC entries and changed nothing. Exact evidence: `docs/deployment/NOMAD_FIRST_WORKLOAD_SHADOW_001.md`. Generic POC long checks were not replayed. | External human action required; no second UAC, no DACL change, no allocation restore, no cutover or Recorder migration |
