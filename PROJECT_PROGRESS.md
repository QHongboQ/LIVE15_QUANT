...
| NOMAD-FIRST-WORKLOAD-SHADOW-001 | PR_PENDING / LOCAL_VALIDATION_COMPLETE | Sealed no-credential ControlCenter shadow is allocated under Nomad with a passing native liveness check. Exact artifact/ACL/allocation evidence: `docs/deployment/NOMAD_FIRST_WORKLOAD_SHADOW_001.md`. Generic POC long checks were not replayed. | Non-Production only; no cutover or Recorder migration |
