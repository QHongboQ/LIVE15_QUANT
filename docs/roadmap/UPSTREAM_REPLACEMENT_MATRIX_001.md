...
| --- | --- |
| **MUST_REPLACE** | Nomad + Windows SCM lifecycle replacement and the React Admin + Material UI Web Application Shell are **ADOPTED / PRODUCTION VERIFIED**; Vector is the next default telemetry candidate (OTel only if a measured OTLP/tracing requirement supersedes it) |
...
| Web application shell | Packaged React terminal served by ControlCenter | [React Admin](https://github.com/marmelab/react-admin) + Material UI | **ADOPTED / PRODUCTION VERIFIED** | Replaced the retired handwritten browser shell, routing, tables, loading/error handling, and theme shell. FastAPI typed projections and read-only domain rules remain local. | The immutable packaged terminal is the sole ControlCenter Web owner; legacy source is retired. |
...
2. Web Application Shell: React Admin + Material UI is **ADOPTED / PRODUCTION VERIFIED**; the
   packaged terminal is the sole ControlCenter Web owner.
