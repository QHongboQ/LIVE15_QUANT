...
| --- | --- |
| **MUST_REPLACE** | Nomad + Windows SCM lifecycle replacement and the React Admin + Material UI Web Application Shell are **ADOPTED / HISTORICAL IMPLEMENTATION**. Recorder and SDK-shadow Nomad ownership remain current; ControlCenter host ownership remains under recovery. Vector is a deferred later candidate (OTel only if a measured OTLP/tracing requirement supersedes it). |
...
| --- | --- | --- | --- | --- | --- |
| Agent and workload lifecycle | Recorder and `kalshi_sdk_ws_shadow` are Nomad-owned; `pyth` and `coinbase` are in-process Recorder workers; RuntimeSupervisor is retired. ControlCenter's historical Nomad cutover does not prove current host ownership. | [HashiCorp Nomad](https://github.com/hashicorp/nomad) + Windows SCM | **ADOPTED / HISTORICAL IMPLEMENTATION** | Replaces generic scheduling, allocation lifecycle, task restart, deployment health, update, and native revert. Does not replace Recorder semantics, risk, or execution. | Historical cold-boot/cutover evidence and final repository cleanup are retained; current ControlCenter host acceptance is a recovery gate. |
| Windows service wrapper | Nomad agent installed as native Windows Service; SCM remains the host authority; RuntimeSupervisor has no current workload role. Legacy WinSW artifacts do not prove a live ControlCenter owner. | Nomad agent installed as native Windows Service; SCM remains the host authority | **ADOPTED / HISTORICAL IMPLEMENTATION** | Replaced the retired per-component RuntimeSupervisor wrapper; shared WinSW bootstrap artifacts may remain without creating a current lifecycle owner. | Historical implementation removes the duplicate wrapper/supervisor path; current ControlCenter ownership requires recovery verification. |
...
| Web application shell | Packaged React terminal implementation served by ControlCenter | [React Admin](https://github.com/marmelab/react-admin) + Material UI | **ADOPTED / HISTORICAL IMPLEMENTATION** | Replaced the retired handwritten browser shell, routing, tables, loading/error handling, and theme shell. FastAPI typed projections and read-only domain rules remain local. | The immutable packaged terminal remains the sole source-level Web owner; current desktop/listener/release operation requires ControlCenter recovery verification. |
...
1. Nomad + Windows SCM lifecycle replacement is **ADOPTED / HISTORICAL IMPLEMENTATION**. Recorder
   and `kalshi_sdk_ws_shadow` are Nomad-owned; current ControlCenter host ownership requires recovery;
   RuntimeSupervisor is retired.
2. Web Application Shell: React Admin + Material UI is **ADOPTED / HISTORICAL IMPLEMENTATION**; the
   packaged terminal remains the sole source-level ControlCenter Web owner, while current desktop and
   listener operation require recovery verification.
3. Vector is the next default telemetry/log candidate, then Grafana if a separate
