...
| Legacy rollback capture boundary | First-deploy legacy capture previously omitted explicit `.worktrees` and release-control-plane exclusions, so a self-capture could recurse. **GUARDED**: capture excludes only the fixed mutable/control-plane boundary set before `copytree` descent; provenance remains `UNPROVEN`. | `tests/test_release_pipeline.py` | `docs/continuous_recorder.md` |
