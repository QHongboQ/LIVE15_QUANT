...
const targetDifference = (now?: string | number | null, target?: string | number | null): TargetDifference | null => {
  ...
  const amount = `${difference > 0 ? '+' : difference < 0 ? '-' : ''}$${Math.abs(difference).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
...
const MarketCard = memo(function MarketCard({ market, trend, onOpen }: { market: Market; trend: ChartPoint[]; onOpen: (id: string) => void }) { const difference = targetDifference(market.underlying_price, market.target); const differenceClass = difference?.label === 'ABOVE TARGET' ? 'positive' : difference?.label === 'BELOW TARGET' ? 'negative' : 'neutral'; return <button className="market-card" onClick={() => onOpen(market.id)}><div className="market-card-top"><strong>{market.asset}</strong><MiniSparkline points={trend} contractStart={market.window_start} contractEnd={market.window_end} /></div><div className="market-card-middle"><span className="market-price">{formatNumber(market.underlying_price ?? market.target)}</span><small className={differenceClass}>{difference ? <>{difference.label} <b>{difference.amount}</b>{difference.percent && ` · ${difference.percent}`}</> : 'Target unavailable'}</small></div><div className="quote-line"><span>UP <b>{probabilityCents(market.yes_bid)}</b></span><span>DOWN <b>{probabilityCents(market.no_bid)}</b></span></div><small>{seconds(market.seconds_remaining)}</small></button>; });
