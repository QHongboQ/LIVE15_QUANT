...
const targetDifference = (now?: string | number | null, target?: string | number | null) => {
  const current = Number(now); const reference = Number(target);
  if (!Number.isFinite(current) || !Number.isFinite(reference)) return 'Target unavailable';
  const difference = current - reference;
  const absolute = `${difference >= 0 ? '+' : ''}${formatNumber(difference)}`;
  return reference === 0 ? `${absolute} vs target` : `${absolute} · ${(difference / reference * 100).toFixed(1)}% vs target`;
};
...
const MarketCard = memo(function MarketCard({ market, trend, onOpen }: { market: Market; trend: ChartPoint[]; onOpen: (id: string) => void }) { const now = Number(market.underlying_price); const target = Number(market.target); const difference = Number.isFinite(now) && Number.isFinite(target) ? now - target : null; return <button className="market-card" onClick={() => onOpen(market.id)}><div className="market-card-top"><strong>{market.asset}</strong><MiniSparkline points={trend} /></div><div className="market-card-middle"><span className="market-price">{formatNumber(market.underlying_price ?? market.target)}</span><small className={difference != null && difference >= 0 ? 'positive' : 'negative'}>{targetDifference(market.underlying_price, market.target)}</small></div><div className="quote-line"><span>UP <b>{probabilityCents(market.yes_bid)}</b></span><span>DOWN <b>{probabilityCents(market.no_bid)}</b></span></div><small>{seconds(market.seconds_remaining)}</small></button>; });
...
function Overview() {
  ...
  return <Page title="Overview" subtitle="Synchronized market state from local LIVE15 authority." status={<Status text={terminalStatus} />}>
...
function Markets() {
  ...
  return <Page title="Markets" subtitle="Live contracts, compact trends, and synchronized Kalshi books."><div className="market-grid">{live.map((market) => {
...
function MarketDetail() {
  ...
  return <Page title={`${data.asset} · 15m`} subtitle="Current authoritative contract" status={<Status text={detailStatusLabel} />}><div className="detail-hero"><Metric label="Now">{formatNumber(data.underlying_price)}</Metric><Metric label="Target">{formatNumber(data.target)}</Metric><Metric label="Difference">{targetDifference(data.underlying_price, data.target)}</Metric><Metric label="UP probability">{probabilityCents(data.yes_bid)}</Metric><Metric label="DOWN probability">{probabilityCents(data.no_bid)}</Metric><Metric label="Remaining">{seconds(data.seconds_remaining)}</Metric></div><div className="terminal-detail"><div className="chart-card"><div className="chart-head"><Box><Typography variant="overline">REALTIME CONTRACT WINDOW</Typography><Typography variant="h6">{mode === 0 ? 'Underlying price' : 'Kalshi probability'}</Typography></Box><Segments value={mode} labels={['PRICE', 'PROBABILITY']} onChange={setMode} /></div>{mode === 0 ? <FinancialChart ariaLabel="Realtime underlying price chart" reference={Number(data.target)} contractStart={data.window_start} resetKey={`${data.ticker}:${history.data.generated_at}:price`} series={[{ id: 'price', label: 'Price', color: '#a68bff', history: historicalPricePoints, livePoints: liveMatchesHistory ? livePricePoints : [] }]} /> : <FinancialChart ariaLabel="Realtime Kalshi probability chart" contractStart={data.window_start} resetKey={`${data.ticker}:${history.data.generated_at}:probability`} series={[{ id: 'yes', label: 'YES', color: '#79e7b1', history: historicalYesPoints, livePoints: liveMatchesHistory ? liveYesPoints : [] }, { id: 'no', label: 'NO', color: '#f0a6cf', history: historicalNoPoints, livePoints: liveMatchesHistory ? liveNoPoints : [] }]} /> }<div className="chart-footer"><span>{detailStatusLabel} · {displayLatency(detailLatency)}</span><span>Last actual change {lastChange ? new Date(lastChange).toLocaleString() : 'unavailable'}</span></div></div><div className="depth-panel"><Depth title="UP bid depth" levels={data.yes_bid_depth} /><Depth title="DOWN bid depth" levels={data.no_bid_depth} /></div></div></Page>;
}
...
function PortfolioSummary() { const [range, setRange] = useState(0); const [custom, setCustom] = useState(false); const [from, setFrom] = useState(''); const [to, setTo] = useState(''); const loader = useCallback(async () => ({ account: await api.accountSummary(), history: await api.accountEquityHistory(custom ? 'ALL' : portfolioRanges[range]) }), [range, custom]); const query = useLazyData(loader); if (query.pending) return <Loading />; if (query.error || !query.data) return <ErrorState error={query.error} retry={query.load} />; const data = query.data.account as Account; const history = query.data.history as EquityHistory; const points = history.points.filter((item) => (!from || item.observed_at >= from) && (!to || item.observed_at <= `${to}T23:59:59Z`)).map((item) => point(item.observed_at, item.portfolio_value_cents == null ? null : item.portfolio_value_cents / 100)).filter((item): item is ChartPoint => item != null); const flatPortfolio = points.length > 0 && points.every((item) => item.value === points[0].value); return <><div className="detail-hero"><Metric label="Total equity">{dollars(data.summary.portfolio_value_cents)}</Metric><Metric label="Cash">{dollars(data.summary.balance_cents)}</Metric><Metric label="Actual samples">{points.length}</Metric><Metric label="Account"><Status text={data.status} /></Metric></div><div className="chart-card"><div className="chart-head"><Box><Typography variant="overline">ACCOUNT VALUE</Typography><Typography variant="h6">Forward-collected equity</Typography></Box><Stack direction="row" gap={1}><Segments value={range} labels={portfolioRanges} onChange={(next) => { setCustom(false); setRange(next); }} /><Button className="quiet-button" onClick={() => setCustom(!custom)}>Custom</Button></Stack></div>{custom && <div className="date-range"><label>From <input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label><label>To <input type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label></div>}<FinancialChart ariaLabel="Account equity history" showLatestMarker={false} hideRightPriceScale={flatPortfolio} series={[{ id: 'equity', label: 'Portfolio', color: '#a68bff', points }]} /><small className="chart-note">{points.length} actual samples · {flatPortfolio ? 'flat series' : 'forward-collected'}</small>{history.notes.map((note) => <small className="chart-note" key={note}>{note}</small>)}</div><Rows rows={data.positions} columns={['ticker', 'position', 'market_exposure_cents', 'realized_pnl_cents', 'mark_cents']} /></>; }
