...
function MarketDetail() {
  ...
  return <Page title={`${data.asset} market`} subtitle={data.ticker ?? 'Awaiting active contract'} refresh={reconcile} status={<Status text={detailStatusLabel} />}><div className="detail-hero"><Metric label="Underlying">{formatNumber(data.underlying_price)}</Metric><Metric label="YES">{formatNumber(data.yes_bid)} / {formatNumber(data.yes_ask)}</Metric><Metric label="NO">{formatNumber(data.no_bid)} / {formatNumber(data.no_ask)}</Metric><Metric label="Remaining">{seconds(data.seconds_remaining)}</Metric><Metric label={mode === 0 ? 'Underlying latency' : 'Projection latency'}>{displayLatency(detailLatency)}</Metric></div><div className="chart-card"><div className="chart-head"><Box><Typography variant="overline">REALTIME CONTRACT WINDOW</Typography><Typography variant="h6">{mode === 0 ? 'Underlying price' : 'Kalshi probability'}</Typography></Box><Segments value={mode} labels={['PRICE', 'PROBABILITY']} onChange={setMode} /></div>{mode === 0 ? <FinancialChart ariaLabel="Realtime underlying price chart" reference={Number(data.target)} resetKey={`${data.ticker}:${history.data.generated_at}:price`} series={[{ id: 'price', label: 'Price', color: '#a68bff', history: historicalPricePoints, livePoints: liveMatchesHistory ? livePricePoints : [] }]} /> : <FinancialChart ariaLabel="Realtime Kalshi probability chart" resetKey={`${data.ticker}:${history.data.generated_at}:probability`} series={[{ id: 'yes', label: 'YES', color: '#79e7b1', history: historicalYesPoints, livePoints: liveMatchesHistory ? liveYesPoints : [] }, { id: 'no', label: 'NO', color: '#f0a6cf', history: historicalNoPoints, livePoints: liveMatchesHistory ? liveNoPoints : [] }]} /> }<div className="chart-footer"><span><span aria-hidden="true">{detailStatus === 'LIVE' ? '●' : '○'}</span> {detailStatusLabel} · {displayLatency(detailLatency)}</span><span>Last actual change {lastChange ? new Date(lastChange).toLocaleString() : 'unavailable'}</span><span>Current point has right-side chart room</span></div></div><div className="detail-grid"><Depth title="YES bid depth" levels={data.yes_bid_depth} /><Depth title="NO bid depth" levels={data.no_bid_depth} /><Card className="surface-card"><CardContent><Typography variant="overline">Market state</Typography><Metric label="Order book"><Status text={data.orderbook_status} /></Metric><Metric label="Underlying">{data.underlying_provider ?? '—'} · {data.underlying_status}</Metric><Metric label="Settlement">{data.settlement_followup}</Metric></CardContent></Card></div></Page>;
}
